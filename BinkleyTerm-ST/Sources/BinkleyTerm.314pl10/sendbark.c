...
int cdecl SBInit( BARKARGSP args, int start_state )
{
   if ( args )
   {
   }
...
int cdecl SBEnd( BARKARGSP args, int cur_state )
{
   if ( args )
   {
   }
...
int cdecl SBSendBark( BARKARGSP args )
{
...
int cdecl SBAskFile( BARKARGSP args )
{
   SENDCHARS (args->barkpacket, args->barklen, 1);
   return (SB2);
}

int cdecl SBRcvFile( BARKARGSP args )
{
   int c;
   int ret;
...
int cdecl SBNxtFile( BARKARGSP args )
{
   int c;
   long SB3Timer;
