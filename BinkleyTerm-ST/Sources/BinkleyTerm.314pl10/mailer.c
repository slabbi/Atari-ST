...
/*
 * Following functions implemented by St.Slabihoud 17.10.1994
*/

static char returnaddr[81];	/* I need the returned string here */

int Overlay_Do_Poll (SAVEP p, int x)
{	REGIONP r;
	if (p != NULL)
		{	r = p->region;
	   	sb_fill (r, ' ', colors.popup);
	   	sb_box (r, boxtype, colors.popup);
			sb_move (r, 0, 1); sb_puts (r, " POLL a node ");
			sb_move (r, 1, 1); sb_puts (r, " Please enter a net/node number:");
			if (sb_input_chars (r, 1, 34, returnaddr, 35, TRUE) && *returnaddr)
				*returnaddr='\0';		/* ESC pressed */
		}
	strcpy(returnaddr,skip_blanks(returnaddr));
	if (*returnaddr)
		return(0);
	else
		return(1);
...
int Do_Poll (SAVEP p, int x)
{	return (Overlay_Do_Poll (p, x)); }

int pressed_altm(char *node)
	{	if (sb_popup (10, 5, 4, 70, Do_Poll, 0))
			{	status_line (":Poll aborted");	/* ESC or empty line */
				return(0);
			}
		else
			{	strcpy(node,returnaddr);				/* return address */
				return(1);
			}
	}
...
int Overlay_Do_Zoom (SAVEP rp, int x)
{
	...
	if (rp != NULL)
	{
		...
		while (!timeup (t_idle) && (PEEKBYTE () == -1))
		{
			if (KEYPRESS ())
			{
				...
				case ALTM:
					...
					if (m >= 1 && nodefind (&next_addr, 1))
					{
							...
							&& (!(newnodedes.NodeFlags & B_CM)))
						{
							...
							if (fullscreen)
							{	status_line (":WARNING: ");
								status_line (":%s",Pretty_Addr_Str(&next_addr));
								status_line (": is not 24 Hours... poll anyway(Y/N)?");
							}
							else
							{	vfossil_cursor (1);
								scr_printf ("\r\n");
...
polling:
					...
					{	/* Name or Node not found; or ESC pressed */
						goto bad_char;
					}
