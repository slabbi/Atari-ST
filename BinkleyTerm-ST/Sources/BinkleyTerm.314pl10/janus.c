...
static unsigned long Diskavail;	  /* Bytes available in upload directory			 */
...
void Janus( void )
{
   ...
   if (((Txfname = malloc(PATHLEN)) == NULL ) || ( Rxfname = malloc(PATHLEN)) == NULL)
   {
   ...
   TimeoutSecs = (unsigned int)(40960U / cur_baud);
   if (TimeoutSecs < 10)
	  TimeoutSecs = 10;
...
/* Sorry, this don't cut in OS/2's zfree */
   ...
   while (p >= Upload_path && *p != '\\')
	  --p;
   *(++p) = '\0';
   if ((Diskavail = zfree(Upload_path)) == 0UL)
	  Diskavail = 0xFFFFFFFFUL;
#else
   Diskavail = (CurrentNetFiles[1] == ':') ? zfree(CurrentNetFiles) : 0x7FFFFFFFL;
#endif
   Rxbufptr = NULL;
			   ...
			   if (xstate == XRCVFNACK) {
				  ...
				  if (Txfname[0]) {
					 ...
					 if ((txpos = *((long *)Rxbuf)) > -1L) {
						...
						if (sending_req) {
						   if ((sending_req = get_reqname(FALSE)) == 0)
							  getfname(GOOD_XFER);
						} else {
