...
static unsigned long last_baud = 0;	/* Current setting */
...
static struct {
	...
	{ 9600,1 },
	{ 19200,0 },
	{ 38400,13 },	/* Same as 2400 baud, but without /16 */
	{ 57600,12 },	/* with rsve */
	{ 76800,15 },
	{ 115200,11 },
	{ 153600,14 },
	{ 0,-1 }			/* End of list.. unchanged rate */
};

unsigned long st_lock_baud = 0;	/* Baud rate above which to force baud rate */
...
void MDM_ENABLE(unsigned long baud)
{
   ...
   if(st_lock_baud && (baud >= st_lock_baud))
	...
	if(baud != last_baud)
	{
		...
		if(baud != 38400 || (baud == 38400 && rsve_board))
			ucr |= 0x80;
#if 0
		else
...
/*		if(rsve_board) rates[16].mask = 13;  BS 92594 */
		i = 0;
