...
address:
		...
		case C_Key:		/* Key [#alias] [%phone] [!password] [&poll_slot] address */
		 ...
		 case C_LockBaud:								/* "LockBaud"	  */
			...
			if (*c)
			   lock_baud = (unsigned long) atol(c);
		 ...
		 case C_JanusBaud:								 /* "JanusBaud"    */
			c = skip_blanks (&temp[9]);
			janus_baud = (unsigned long) atol(c);
			break;
		...
		case C_STlockBaud:								/* STlockBaud */
			...
			if (*c)
			   st_lock_baud = (unsigned long) atol(c);
		...
		if ((toupper(*c) >= 'A') && (toupper(*c) <= 'F'))
		 ...
		 case C_NeedTime:
			c = skip_blanks (&temp[8]);
			needtime = atoi (c);
			if (needtime<0 || needtime>1000)
				{	needtime=0;
					(void) printf ("Illegal config line: %s\n", temp);
				}
			break;
